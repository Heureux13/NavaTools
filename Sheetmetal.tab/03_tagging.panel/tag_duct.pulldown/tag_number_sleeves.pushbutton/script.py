...
ANNOTATION_TAG_CANDIDATES = list(
    DEFAULT_TAG_SLOT_CANDIDATES.get(SLOT_NUMBER_SLEEVE, []))
SIZE = RVT_SIZE
...
def _build_sleeve_opening_value(element):
    ...
    rule = NORMALIZED_CONNECTOR_RULES.get(
        connector_key, DEFAULT_CONNECTOR_RULE)
...
if isinstance(annotation_tag_name, tuple):
    annotation_tag_name = '{} : {}'.format(
        annotation_tag_name[0], annotation_tag_name[1])
annotation_tag_family = ''
if annotation_tag_symbol is not None and annotation_tag_symbol.Family is not None:
    annotation_tag_family = (
        annotation_tag_symbol.Family.Name or '').strip().lower()

existing_annotation_tag_map = {}
if annotation_tag_symbol is not None and annotation_tag_family:
    existing_annotation_tag_map = tagger.build_existing_tag_family_map(sleeves)
...
with revit.Transaction('Number sleeve elements'):
    ...
    for element in sleeves:
        ...
        try:
            annotation_tag = tagger.place_tag_at_center_with_rotation(
                element,
                annotation_tag_symbol,
                position='center'
            )
...
output.print_md('Already numbered in `{}` (kept): {}'.format(
    NUMBER_PARAM, len(already_numbered)))
output.print_md('Newly numbered in `{}`: {}'.format(
    NUMBER_PARAM, len(numbered)))
if has_tag_param:
...
if missing_type:
    output.print_md('Non-sleeve elements without `{}` (skipped): {}'.format(
        TYPE_PARAM, len(missing_type)))
if failed:
    output.print_md('Failed to write `{}`: {}'.format(
        NUMBER_PARAM, len(failed)))
if has_tag_param and tag_failed:
    output.print_md('Failed to write `{}`: {}'.format(
        TAG_PARAM, len(tag_failed)))
if has_tag_param and tag_param_missing:
    output.print_md('Missing parameter `{}`: {}'.format(
        TAG_PARAM, len(tag_param_missing)))
if has_tag_param and tag_param_readonly:
    output.print_md(
        'Read-only parameter `{}`: {}'.format(TAG_PARAM, len(tag_param_readonly)))
if opening_failed:
    output.print_md('Failed to write `{}`: {}'.format(
        SLEEVE_OPENING, len(opening_failed)))
if opening_no_value:
    output.print_md('No computed opening value: {}'.format(
        len(opening_no_value)))
if size_parse_failed:
    output.print_md('Failed to parse `{}` size: {}'.format(
        SIZE, len(size_parse_failed)))
    size_failed_ids = [e.Id for e in size_parse_failed]
    output.print_md('Size parse failed elements: {}'.format(
        output.linkify(size_failed_ids)))
if opening_param_missing:
    output.print_md('Missing parameter `{}`: {}'.format(
        SLEEVE_OPENING, len(opening_param_missing)))
if opening_param_readonly:
    output.print_md(
        'Read-only parameter `{}`: {}'.format(SLEEVE_OPENING, len(opening_param_readonly)))
