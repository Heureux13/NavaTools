...
def _resolve_bic_value(name):
    """Return int value for a BuiltInCategory name when available."""
    try:
...
MEP_TAGGED_ELEMENT_CATEGORY_IDS = set(
    [
        val for val in
        (_resolve_bic_value(name) for name in MEP_TAGGED_CATEGORY_NAMES)
        if val is not None
    ]
)
...
def is_mep_tag(tag):
    ...
    for tagged_id in tagged_ids:
        elem = doc.GetElement(tagged_id)
        if not elem or not elem.Category:
            continue

        cat_id_value = _get_element_id_value(elem.Category.Id)
        if cat_id_value is not None and cat_id_value in MEP_TAGGED_ELEMENT_CATEGORY_IDS:
            return True
...
try:
    ...
    mep_tags = [t for t in mep_tags if t.OwnerViewId ==
                source_view.Id and is_mep_tag(t)]

    if not mep_tags:
        output.print_md('## No MEP tags found in the active view.')
        script.exit()

    by_family = collect_annotation_options(mep_tags)
    ...
    if not matching_tags:
        output.print_md(
            '## No matching MEP tags found for the selected annotation(s).')
        script.exit()
    ...
    picked = forms.SelectFromList.show(
        ...
        multiselect=True,
        title='Select Views to Paste MEP Tags'
    )
    if not picked:
        script.exit()
    ...
    with revit.Transaction('Copy MEP Tags To Views'):
    ...
    output.print_md(
        '### Selected {} MEP tags and pasted to {} view(s).'.format(
            len(matching_tags),
            copied_count,
        )
    )
