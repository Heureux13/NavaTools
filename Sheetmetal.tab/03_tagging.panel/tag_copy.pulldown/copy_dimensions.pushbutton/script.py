...
def get_dimension_family_and_type(dimension):
    family_name = None
    type_name = None
    ...
    if dim_type:
        try:
            family_name = getattr(dim_type, 'FamilyName', None)
        except Exception:
            family_name = None
        ...
        if not family_name:
            try:
                fam_param = dim_type.get_Parameter(
        ...
        if not type_name:
            try:
                name_param = dim_type.get_Parameter(
...
def collect_dimension_options(dimensions):
    """Group dimensions by family, then by type within each family."""
    by_family = {}
    for dimension in dimensions:
        family_name, type_name = get_dimension_family_and_type(dimension)
        ...
        by_family[family_name][type_name].append(dimension)

    return by_family


def select_dimension_families_and_types(by_family):
    ...
    class TagSelectionForm(Form):
        ...
        def get_checked_types(self):
            """Get all checked dimension type keys."""
            result = set()
...
try:
    ...
    dimensions = [d for d in dimensions if d.OwnerViewId == source_view.Id]

    if not dimensions:
        output.print_md('## No dimensions found in the active view.')
        script.exit()

    by_family = collect_dimension_options(dimensions)
    selected_keys = select_dimension_families_and_types(by_family)
    ...
    if not matching_dimensions:
        output.print_md(
            '## No matching dimensions found for the selected type(s).')
        script.exit()

    selected_ids = List[ElementId]([dim.Id for dim in matching_dimensions])
    ...
    picked = forms.SelectFromList.show(
        ...
        multiselect=True,
        title='Select Views to Paste Dimensions'
    )
    if not picked:
        script.exit()
    ...
    with revit.Transaction('Copy Dimensions To Views'):
    ...
    output.print_md(
        '### Selected {} dimensions and pasted to {} view(s).'.format(
            len(matching_dimensions),
            copied_count,
        )
    )
