...
if not sel_ids:
    output.print_md('## Select duct to start numbering')
    script.exit()

start_elem = doc.GetElement(list(sel_ids)[0])
if start_elem is None:
    output.print_md(
        "## Could not read selected element. Select one fabrication duct.")
    script.exit()
...
if start_number is None:
    start_number = 1
    output.print_md(
        "## Selected start duct has no Item Number. Starting numbering at 1.")
...
try:
    ...
    start_number = 1
    output.print_md(
        "## Selected start duct Item Number is invalid. Starting numbering at 1.")
...
t_num = Transaction(doc, "Number Item Numbers")
t_num.Start()

try:
    numbering.number_duct(start_duct,
    ...
    t_num.RollBack()
    output.print_md("## Numbering error: {}".format(e))
    raise
...
try:
    ...
    for elem in fab_ducts:
        ...
        # Skip elements with no usable item number (using RevitNumbers API)
        try:
            elem_duct = RevitDuct(doc, view, elem)
            item_number = numbering.get_item_number(elem_duct)
            if item_number is None or int(item_number) <= 0:
