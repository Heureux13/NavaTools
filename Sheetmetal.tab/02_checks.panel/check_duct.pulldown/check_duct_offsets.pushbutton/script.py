...
for d in all_duct:
    ...
    offset_param = d.LookupParameter(
        PYT_OFFSET_VALUE) or d.LookupParameter(LEGACY_OFFSET)
    offset_str = ""
    if offset_param:
...
if not grouped_elements:
    forms.alert("No non-CL offset elements found in current view.",
                exitscript=True)
...
for offset_value in sorted(grouped_elements.keys(), key=lambda x: x.lower()):
    output.print_md("## {} ({})".format(
        offset_value, len(grouped_elements[offset_value])))
    output.print_md("---")
