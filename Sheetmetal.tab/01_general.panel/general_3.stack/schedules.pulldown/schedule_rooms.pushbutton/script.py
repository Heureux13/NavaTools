...
# Button info
# ======================================================================
__title__ = 'Populate Schedule Rooms'
__doc__ = '''
...
def parse_length_inches(value_text):
    """Parse common feet-inch formats and return inches as float.

    Supported examples:
      - 09-00
      - 9'-0"
      - 9'0"
      - 108 (treated as inches)
    """
    text = clean_cell_value(value_text)
    if not text:
        return 0.0

    normalized = text.replace(' ', '').replace('"', '').replace('”', '').replace('′', "'").replace('’', "'")

    # Explicit feet marker, e.g. 9'-0 or 9'0
    if "'" in normalized:
        feet_part, inches_part = normalized.split("'", 1)
        feet = _safe_float(feet_part)
        if feet is None:
        ...
        inches = _safe_float(inches_part) if inches_part else 0.0
        if inches is None:
            inches = 0.0
        return (feet * 12.0) + inches

    # Hyphen format, e.g. 09-00 (feet-inches).
    if '-' in normalized:
        parts = normalized.split('-')
        if len(parts) == 2:
            feet = _safe_float(parts[0])
            inches = _safe_float(parts[1])
            if feet is not None and inches is not None:
                return (feet * 12.0) + inches

    # Fallback: treat plain numeric input as inches.
    return parse_number(text)
        ...
        if storage == StorageType.String:
            ...
            if value_text:
                if is_length_parameter(param):
                    numeric_value = parse_length_inches(value_text)
                else:
