...
def copy_titleblock_instance_parameters(source_tb, target_tb):
    # Keep target sheet identity fields generated by this script.
...
def copy_reference_sheet_content(document, source_sheet, target_sheet):
    ...
    for element in source_elements:
        ...
        # Avoid copying model elements to a sheet view.
        try:
    ...
    try:
        ...
        # If one element is invalid for copy, continue copying the rest.
        for element_id in ids_to_copy:
            one_id = List[ElementId]()
            one_id.Add(element_id)
            try:
                ElementTransformUtils.CopyElements(
                    source_sheet,
                    one_id,
...
def choose_best_reference(target_view, candidates):
    if not candidates:
        return None
...
def flatten_reference_candidates(reference_index):
    all_candidates = []
    for candidates in reference_index.values():
        all_candidates.extend(candidates)
    return all_candidates


def choose_reference_for_target_view(target_view, reference_index, all_candidates):
    """
    Prefer exact scope-box match, then fall back to best overall reference view.
    This allows one reference viewport/sheet to drive many target views.
    """
    scope_key = get_scope_box_key(target_view)
    if scope_key:
        match = choose_best_reference(target_view, reference_index.get(scope_key, []))
        if match:
            return match

    return choose_best_reference(target_view, all_candidates)
...
if not reference_index:
    ...
    script.exit()
all_reference_candidates = flatten_reference_candidates(reference_index)
...
with revit.Transaction('Create Sheets'):
    for target_view in selected_views:
        ...
        try:
            ...
            if not scope_suffix:
                # Fall back to target view name when no scope box is assigned.
                scope_suffix = ''.join(ch for ch in (target_view.Name or '').upper() if ch.isalnum())
            if not scope_suffix:
                scope_suffix = str(target_view.Id.IntegerValue)

            match = choose_reference_for_target_view(
                target_view, reference_index, all_reference_candidates)
            if not match:
                raise Exception('no reference viewport found on selected reference sheets')
