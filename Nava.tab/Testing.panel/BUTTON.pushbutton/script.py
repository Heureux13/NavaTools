...
# Button info
# ===================================================
__title__ = 'Spiral Under 03" Long'
__doc__ = """
placeholder
"""
...
fil_ducts = [
    d for d in ducts
    if d.length and d.length < SPIRAL_LENGTH and d.family and d.family.strip().lower() in acceptable
]
...
if fil_ducts:
    ...
    for i, fil in enumerate(fil_ducts, start=1):
        output.print_md(
            '### Index: {:02} | Family: {} | Size: {} | Length: {:02}" | Element ID: {}'.format(
    ...
    output.print_md('## No spiral under 03" found')
