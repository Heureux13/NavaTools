...
# Pre-fetch BOD tag family name for already-tagged checks
try:
    _bod_tag_symbol = RevitTagging(doc, view).get_label("_umi_bod")
    _bod_tag_fam = (
        _bod_tag_symbol.Family.Name if _bod_tag_symbol and _bod_tag_symbol.Family else "").strip()
except Exception:
    _bod_tag_symbol = None
    _bod_tag_fam = ""
...
try:
    ...
    def build_tag_plan(straights, run_builder):
        ...
        while True:
            ...
            def has_bod_tag(duct):
                try:
                    return tagger.already_tagged(duct.element, _bod_tag_fam)
                except Exception:
                    return False
    ...
    bod_selected_ids, bod_considered_ids, bod_run_infos, bod_run_count = build_tag_plan(
        all_straights,
        RevitDuct.create_duct_run_same_height,
    )

    output.print_md("**BOD runs: {} runs, {} straights chosen for BOD tags**".format(
        bod_run_count, len(bod_selected_ids)))

    # Build tag plan focused on BOD tags
    tag_plan = {}
    for eid in bod_selected_ids:
        tag_plan[eid] = ['tag_bod']

    connected_straights_to_tag = [elem_lookup[eid]
                                  for eid in tag_plan.keys() if eid in elem_lookup]

    considered_ids = bod_considered_ids
    ...
    bod_assign_count = sum(1 for v in tag_plan.values() if 'tag_bod' in v)
    bod_place_count = 0
    ...
    output.print_md(
        "# BOD assigned/placed: {}/{}".format(bod_assign_count, bod_place_count))
    if already_tagged:
