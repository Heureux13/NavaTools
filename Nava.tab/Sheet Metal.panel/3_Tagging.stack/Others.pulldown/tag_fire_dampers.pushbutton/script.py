...
# =================================================
__title__ = "Tag Fire Dampers"
__doc__ = """
Tags fire damper sleeve fabrication ductwork in the current view with MARK label tags.
"""
...
def _is_fire_damper_sleeve(elem, doc):
    type_value = _normalized(_get_param_value(elem, TYPE_PARAM_NAME))
    if type_value != TYPE_PARAM_VALUE:
        return False

    for pname in FIRE_DAMPER_PARAM_NAMES:
        if FIRE_DAMPER_TOKEN in _normalized(_get_param_value(elem, pname)):
            return True

    if FIRE_DAMPER_TOKEN in _normalized(_get_element_family_name(elem, doc)):
        return True
    ...
    try:
        ...
        if type_elem is not None:
            for pname in FIRE_DAMPER_PARAM_NAMES:
                if FIRE_DAMPER_TOKEN in _normalized(_get_param_value(type_elem, pname)):
                    return True
    except Exception:
        pass
...
equipment_elements = [
    ...
    if _is_fire_damper_sleeve(e, doc)
]

if not equipment_elements:
    output.print_md("## No fire damper sleeve fabrication ductwork found in this view.")
    script.exit()
...
t = Transaction(doc, "Tag Fire Dampers Fabrication Ductwork MARK")
t.Start()
try:
