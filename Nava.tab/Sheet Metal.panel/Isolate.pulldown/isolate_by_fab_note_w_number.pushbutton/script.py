...
class EnhancedParamForm(Form):
    def __init__(self, param_groups):
        ...
        self.chk_fab_exported.Top = 415
        self.chk_fab_exported.Left = 310
...
try:
    ...
    # Find all tags associated with the selected ducts
    ...
    for tag in all_tags:
        try:
            # Try different methods to get the tagged element
            tagged_id = None
    ...
    if all_ids.Count == 0:
        output.print_md("**ERROR: Nothing to isolate!**")
        script.exit()

    # Isolate selected ducts and tags in the active view
    try:
        t = Transaction(doc, "Temporary Isolate Ducts")
        t.Start()
        view.IsolateElementsTemporary(all_ids)
        t.Commit()
    except Exception:
        try:
            t.RollBack()
